...
let pdfArray = ["https://developer.apple.com/programs/terms/apple_developer_agreement.pdf",
                "https://itunesconnect.apple.com/docs/iBookstoreAssetGuide5.0.pdf",
                "https://www.iso.org/iso/annual_report_2009.pdf"]
...
class HomeTableViewController: UITableViewController, cellDelegate {
    ...
    func downloadFileWithIndex(ind:Int) {
        ...
        Alamofire.download(urlString, to: destination).downloadProgress(closure: { (prog) in hud.progress = Float(prog.fractionCompleted)
